...
- `GET /api/stats` — relay stats for the admin page. Returns `{ version, claimed, ownerPubkey, totalEvents, events24h, ingested24h, rowsWrittenToday, liveAt, storageBytes, storageBytesLimit, dailyRowsWrittenLimit, dailyRowsReadLimit, backfill, icon, relayName, writePolicy, followCount, followsListAt, vanishing, reads }`. `events24h` counts events by their own timestamp, which is what you posted; `ingested24h` counts what this relay actually took in, backfill included. During a backfill those differ by orders of magnitude.
  `totalEvents`, `events24h` and `followCount` are maintained counters, exact and current as of the request. `events24h`'s window is whole hours, so it spans 24–25 hours rather than exactly 24.
  One field is a timestamp for others: `ingested24h` and `rowsWrittenToday` are computed at most every five minutes and dated by `liveAt`, because they cost more to produce than an unauthenticated endpoint can afford per request — see CLAUDE.md "The budget". Everything else is read live. There used to be a second timestamp, `snapshotAt`, dating a six-hour cache over the counts that walked a table; it was removed along with that cache when those counts became maintained.

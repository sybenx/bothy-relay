...
- `GET /api/stats` — relay stats for the admin page. Returns `{ version, claimed, ownerPubkey, totalEvents, events24h, snapshotAt, ingested24h, rowsWrittenToday, liveAt, storageBytes, storageBytesLimit, dailyRowsWrittenLimit, dailyRowsReadLimit, backfill, icon, relayName, writePolicy, followCount, followsListAt, largestNonOwnerAuthor, vanishing, reads }`. `events24h` counts events by their own timestamp, which is what you posted; `ingested24h` counts what this relay actually took in, backfill included. During a backfill those differ by orders of magnitude.
  Two of these fields are timestamps for the others, because the document describes two instants rather than one. `totalEvents`, `events24h`, `followCount`, `followsListAt` and `largestNonOwnerAuthor` are computed at most every six hours and dated by `snapshotAt`; `ingested24h` and `rowsWrittenToday` are computed at most every five minutes and dated by `liveAt`. Both figures cost more to produce than an unauthenticated endpoint can afford per request — see CLAUDE.md "The budget". Everything else is read live.

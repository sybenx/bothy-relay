...
## NIP-86 phase two — banpubkey/allowpubkey

Phase one's baseline, taken against the deployed relay before this work started: 10k requests, 1M rows read, 33k rows written, 5.49 MB stored (billing figures — the dashboard's Metrics-tab storage chart disagreed with billing and billing is the one to trust).

Phase two adds exactly the per-event lookup phase one deferred, and nothing else:

- **`banned_pubkeys` costs +1 indexed rows-read lookup on every non-owner write.** `ownership.ts isAllowedWriter` calls `storage.ts isPubkeyBanned` before the follows check, so a banned pubkey is refused even if it's also in the owner's follow list. The owner's own pubkey short-circuits before this (`pubkey === owner` returns allowed first), so the owner never pays this lookup, and `src/nip86.ts banpubkey` refuses outright to ever put the owner's pubkey in the table — there's no path where the owner's own writes could be blocked by this.
- **`allowed_pubkeys` costs +1 indexed rows-read lookup only on the path already about to reject a write** — owner-only mode, or "not a follow" under `ALLOW_FOLLOWS`. It never runs for an owner write or an existing follow's write, which is the common case. This is a separate list from `banned_pubkeys`, not its inverse: banning and manually allowing a pubkey are independent operations, each with its own `un-` method (`unbanpubkey`, `unallowpubkey`) for symmetry with `blockip`/`unblockip` — unlike `banevent`/`allowevent`, where `allowevent` lifts a `banevent` tombstone rather than adding to a separate list.
